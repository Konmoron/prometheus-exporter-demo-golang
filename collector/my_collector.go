...
func NewMetrics(namespace string) *Metrics {
	return &Metrics{
		metrics: map[string]*prometheus.Desc{
			"my_counter_metric": newGlobalMetric(namespace, "my_counter_metric", "The description of my_counter_metric", []string{"host", "name"}),
...
func (c *Metrics) Collect(ch chan<- prometheus.Metric) {
	...
	for host, currentValue := range mockCounterMetricData {
		...
		// c.Collect()
		ch <- prometheus.MustNewConstMetric(c.metrics["my_counter_metric"], prometheus.CounterValue, float64(currentValue), host, host)
	}
 ...
 * 功能：生成模拟数据
 */
func (c *Metrics) GenerateMockData() (mockCounterMetricData map[string]int64, mockGaugeMetricData map[string]int) {
	mockCounterMetricData = map[string]int64{
		"yahoo.com":  time.Now().Unix() + int64(rand.Int31n(1000)),
		"google.com": time.Now().Unix() + int64(rand.Int31n(1000)),
	}
